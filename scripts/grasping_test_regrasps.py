...
from planning.regrasp_display_planner import RegraspPlanner
from planning.turntable_planner import TurntablePlanner
from perception.image_saver import ImageSaver
from perception.mask_extractor import MaskExtractor
...
class ObjectPoseExtractor(LeafSystem):
    def __init__(
            self,
            object_ind,
        ):
...
def start_scenario(
    ...
    if not use_hardware:
        ...
        builder.Connect(
            ...
            planner.GetInputPort("object_pose"),
        )
...
if __name__ == "__main__":
    ...
    if args.use_hardware:
        raise NotImplementedError("Hardware not supported yet for regrasps")
