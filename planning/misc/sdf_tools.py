...
class SignedDensityField(object):
    ...
    def __init__(self, data, origin, delta):
        ...
        self.data_torch = torch.from_numpy(self.data.astype(np.float32))
        self.delta_torch = torch.FloatTensor([delta]).float()
        self.origin_torch = torch.from_numpy(self.origin).float()
        self.min = torch.LongTensor([0, 0, 0])
        self.max = torch.LongTensor([self.nx - 1, self.ny - 1, self.nz - 1])
    ...
    def _rel_pos_to_idxes(self, rel_pos):
        i_min = np.array([0, 0, 0], dtype=int)
        i_max = np.array([self.nx - 1, self.ny - 1, self.nz - 1], dtype=int)
        idx = ((rel_pos - self.origin) / self.delta).astype(int)
        return np.clip(idx, i_min, i_max)

    def _rel_pos_to_idxes_torch(self, rel_pos):
        return torch.min(
            torch.max(((rel_pos - self.origin_torch) / self.delta_torch).long(), self.min),
            self.max,
        )
    ...
    def trim(self, dim, center=True):
        ...
        self.origin = self.origin + np.array([x_padding, y_padding, z_padding]) * self.delta
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", action="store_true", help="Visualize signed density field as voxels.")
    ...
    if args.n:
        ...
        voxel = (np.abs(sdf.data) <= 0.01).astype(int)
