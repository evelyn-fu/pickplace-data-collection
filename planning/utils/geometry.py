...
def unpack_action_pytorch3d(pose):
    pose_mat = torch.eye(4, device=pose.device).repeat(len(pose), 1, 1)
    pose_mat[:, :3, 3] = pose[:, :3]
    pose_mat[:, :3, :3] = to_rotation_matrices(pose[:, 3:])
    return pose_mat
...
def to_rotation_matrices(angles: torch.Tensor) -> torch.Tensor:
    """Converts roll, pitch, yaw angles into a batch of rotation matrices.

    Args:
        angles (torch.Tensor): Roll, pitch, yaw angles of shape (...,3).

    Returns:
        torch.Tensor: Rotation matrices of shape (...,3,3).
    """
    roll, pitch, yaw = angles[..., 0], angles[..., 1], angles[..., 2]
    batch_dims = list(roll.shape)
    ...
    # Construct 2D rotation matrices
    ...
    # Construct 3D rotation matrices from 2D rotation matrices
