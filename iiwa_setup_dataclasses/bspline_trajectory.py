from dataclasses import dataclass
import numpy as np
from typing import Optional, List
from pathlib import Path
import os
from pydrake.all import (
    BsplineBasis,
    BsplineTrajectory,
    CompositeTrajectory,
    BezierCurve
)

@dataclass
class BsplineTrajectoryAttributes:
    """A data class to hold the attributes of a B-spline trajectory."""

    spline_order: int
    """The order of the B-spline basis to use."""
    control_points: np.ndarray
    """The control points of the B-spline trajectory of shape
    (num_joints, num_control_points)."""
    knots: np.ndarray
    """The knots of the B-spline basis of shape (num_knots,)."""

    @classmethod
    def from_bspline_trajectory(cls, traj: BsplineTrajectory) -> None:
        """Sets the attributes from a B-spline trajectory."""
        assert traj.start_time() == 0.0, "Trajectory must start at time 0!"
        return cls(
            spline_order=traj.basis().order(),
            control_points=traj.control_points(),
            knots=np.array(traj.basis().knots()) * traj.end_time(),
        )

    def log(self, logging_path: Path) -> None:
        """Logs the B-spline trajectory attributes to disk."""
        np.save(logging_path + "/spline_order.npy", np.array([self.spline_order]))
        np.save(logging_path + "/control_points.npy", self.control_points)
        np.save(logging_path + "/knots.npy", self.knots)

    @classmethod
    def load(cls, path: Path) -> "BsplineTrajectoryAttributes":
        """Loads the B-spline trajectory attributes from disk."""
        return cls(
            spline_order=int(np.load(path + "/spline_order.npy")[0]),
            control_points=np.load(path + "/control_points.npy"),
            knots=np.load(path + "/knots.npy"),
        )

    def to_bspline_trajectory(self) -> BsplineTrajectory:
        """Converts the attributes to a B-spline trajectory."""
        return BsplineTrajectory(
            basis=BsplineBasis(order=self.spline_order, knots=self.knots),
            control_points=self.control_points,
        )
    
@dataclass
class CompositeBezierCurveTrajectoryAttributes:
    """A data class to hold the attributes of a Composite trajectory of Bezier curves."""

    control_points: List[np.ndarray]
    """List of the control points of the Bezier Curve of shape
    (num_joints, num_control_points) for each Bezier Curve."""
    start_times: List[float]
    """List of the start times for each Bezier Curve"""
    end_times: List[float]
    """List of the end times for each Bezier Curve"""

    @classmethod
    def from_composite_bezier_curve_trajectory(cls, traj: CompositeTrajectory) -> "CompositeBezierCurveTrajectoryAttributes":
        """Sets the attributes from a Composite trajectory of Bezier Curves."""
        control_points = []
        start_times = []
        end_times = []
        for i in range(traj.get_number_of_segments()):
            segment = traj.segment(i)
            control_points.append(segment.control_points())
            start_times.append(segment.start_time())
            end_times.append(segment.end_time())
        
        return cls(
            control_points=control_points,
            start_times=start_times,
            end_times=end_times
        )

    def log(self, logging_path: Path) -> None:
        """Saves the composite trajectory attributes to disk"""
        for i in range(len(self.start_times)):
            np.save(logging_path + f"/control_points/{i}.npy", self.control_points[i])
            np.save(logging_path + f"/start_times/{i}.npy", self.start_times[i])
            np.save(logging_path + f"/end_times/{i}.npy", self.end_times[i])

    @classmethod
    def load(cls, path: Path) -> "CompositeBezierCurveTrajectoryAttributes":
        """Loads the composite trajectory attributes from disk."""
        control_points = []
        start_times = []
        end_times = []
        subpath = os.path.join(path, "control_points")
        num_segments = len([entry for entry in os.listdir(subpath) if os.path.isfile(os.path.join(subpath, entry))])
        for i in range(num_segments):
            control_points.append(np.load(path + f"/control_points/{i}.npy"))
            start_times.append(np.load(path + f"/start_times/{i}.npy"))
            end_times.append(np.load(path + f"/end_times/{i}.npy"))

        return cls(
            control_points=control_points,
            start_times=start_times,
            end_times=end_times
        )

    def to_composite_bezier_curve_trajectory(self) -> CompositeTrajectory:
        """Converts the attributes to a Composite trajectory of Bezier Curves."""
        segments = []
        for i in range(len(self.start_times)):
            segments.append(BezierCurve(
                start_time=self.start_times[i],
                end_time=self.end_times[i],
                control_points=self.control_points[i],
            ))
        
        return CompositeTrajectory(segments)